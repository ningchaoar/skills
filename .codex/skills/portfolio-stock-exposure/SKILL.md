...
description: 将用户上传图片或粘贴文本中的基金和股票持仓，实时转换为股票维度的穿透持仓占比；当用户要求把某只股票提高或降低到目标仓位时，计算机械调仓所需的股票或基金交易量。适用于 A 股、中国内地公募基金和中国内地上市 ETF 持仓截图或文字，也支持直接持有港股、美股时按公开价格和汇率换算为人民币市值后统计；基金成分股可包含东方财富返回的 A 股、港股和美股。除非用户明确扩展范围，否则不要用于海外 ETF 或全球基金独立穿透。
---
...
运行代码时优先使用 `scripts/` 下的既有入口。常规持仓分析不得在项目根目录、工作目录或临时目录生成长篇幅 Python 文件，不得复制 `portfolio_math.py`、`query_state.py`、`fund_components.py` 或 `exposure_report.py` 的函数实现。只有现有脚本无法覆盖用户明确要求的新计算时，才可以新增可复用脚本到本技能的 `scripts/` 目录，并同时补测试。

脚本入口边界：

| 脚本 | 用法 |
|---|---|
| `scripts/position_values.py` | 可直接运行；补全直接股票人民币市值 |
| `scripts/fund_components.py` | 可直接运行；查询并写回基金成分股 |
| `scripts/exposure_report.py` | 可直接运行；输出最终持仓表或 JSON |
| `scripts/market_data_probe.py` | 仅手动运行；探测行情和汇率数据源 |
| `scripts/eastmoney_interface_probe.py` | 仅手动运行；探测基金持仓页面稳定性 |
| `scripts/query_state.py` | 只供 import；读写单一临时查询文件 |
| `scripts/market_data.py` | 只供 import；行情、汇率和 `secid` 查询函数 |
| `scripts/portfolio_math.py` | 只供 import；穿透持仓和机械调仓计算 |
| `scripts/script_utils.py` | 只供 import；共享数值、市场和来源汇总 helper |

常规持仓分析只运行前三个可直接运行入口。不要把只供 import 的脚本当作命令入口运行。

默认范围：

- 中国内地公募基金和中国内地上市 ETF
- A 股、港股、美股直接股票持仓
- 东方财富/天天基金可返回人民币持仓市值的港股、美股基金成分股
- 人民币 `CNY` 作为组合统计基准

默认不处理：

- 海外 ETF、海外基金、全球基金的独立数据源查询
- 税费、交易成本、盘中实时性保证
- `CNY/HKD/USD` 以外货币的自动换算
- 债券、现金、衍生品的进一步拆分
...
- `name`：基金或股票名称
- `symbol`：基金代码或股票代码
- `market_value`：当前持仓人民币市值；直接股票缺失时可由 `quantity` 和行情/汇率脚本补全
...
每次写入 `tmp/latest_query.json` 后先运行：
...
- 从 `tmp/latest_query.json` 读取 `current_positions`
- 对直接股票补全 `market`、`currency`、`current_price`、`local_market_value`、`fx_rate_to_cny`、`market_value`
- 已有 `market_value` 时不联网估值，不覆盖该字段
- 缺少 `market_value` 但有 `quantity` 时，按公开行情和汇率换算为人民币市值
- 将结果写回同一个 `tmp/latest_query.json`

如果行情或汇率查询失败，不要猜测。停止并只输出需要用户补充的字段，例如人民币市值、本币价格或汇率。
...
最终计算必须调用 Python，不要手算最终百分比。常规持仓分析直接运行：
...
- 从 `tmp/latest_query.json` 读取 `current_positions` 和 `fund_components`
- 调用 `portfolio_math.compute_stock_exposure()`
- 输出可直接用于最终回答的 Markdown 持仓表

如果需要检查结构化结果，运行：

```bash
python scripts/exposure_report.py --format json
```

只有在调仓计算或特殊输出需要时，才短代码导入 helper；不要复制 helper 实现：
...
3. 运行 `python scripts/position_values.py`
4. 查询基金成分股
5. 运行 `python scripts/exposure_report.py`

### A2. 整理输出数据

优先直接使用 `scripts/exposure_report.py` 输出的表格。不要为了格式化表格临时创建新的 Python 文件。
...
|---|---|---|---:|---:|---|---|
| 300750 | 宁德时代 | CN | xxx.xx | xx.xx% | 直接持股 + 基金穿透 | 2025-12-31 |
